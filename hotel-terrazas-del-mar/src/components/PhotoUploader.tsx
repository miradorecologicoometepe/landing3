...
export const PhotoUploader: React.FC<Props> = ({ rooms, onUploaded }) => {
  ...
  const publish = async (event: React.FormEvent) => {
    ...
    if (category === 'rooms' && !roomId) { setStatus('Selecciona la habitación.'); return; }
    ...
    try {
      ...
      if (admin.error || !admin.data) throw new Error('Tu usuario aún no tiene autorización para publicar fotografías.');
      for (let i = 0; i < files.length; i++) {
        ...
        const photo: PhotoItem = { id, title: (files.length === 1 && title.trim()) || file.name.replace(/\.[^.]+$/, ''), category, url, caption: '', roomTypeId: category === 'rooms' ? roomId : undefined, aspectRatio: 'landscape' };
        ...
        if (category === 'rooms') {
          const room = await supabase.from('rooms').select('details').eq('id', roomId).maybeSingle();
        ...
        onUploaded(photo, category === 'rooms' ? roomId : '');
        setProgress(i + 1);
      }
      setStatus(`Se publicaron ${files.length} fotografía(s). Los visitantes verán los cambios al recargar.`);
      setFiles([]); setTitle('');
    } catch (err) { setStatus(err instanceof Error ? err.message : 'No se pudo publicar.'); }
  ...
  return <form onSubmit={publish} className="bg-white border border-teal-200 rounded-2xl p-5 space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <input type="email" autoComplete="username" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Correo de administrador Supabase" aria-label="Correo de administrador Supabase" className="border rounded-xl p-3 text-sm" />
      <input type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Contraseña Supabase" aria-label="Contraseña Supabase" className="border rounded-xl p-3 text-sm" />
    </div>
    ...
    {category === 'rooms' && <label className="block text-sm font-semibold text-stone-800">Habitación
      <select value={roomId} onChange={e=>setRoomId(e.target.value)} className="block w-full mt-1 border rounded-xl p-3">{rooms.map(room=><option key={room.id} value={room.id}>{room.name}</option>)}</select>
    </label>}
    ...
    <label className="block text-sm font-semibold text-stone-800">Fotografías JPG, PNG, WebP o AVIF (máximo 10 MB por foto)
      <input type="file" accept="image/jpeg,image/png,image/webp,image/avif" multiple onChange={e=>setFiles(Array.from(e.target.files || []))} className="block w-full mt-2 text-sm" />
    </label>
